...
function PagePreview({ page }: { page: string }) {
  ...
  const rows = [
    ...
    'Tables, forms, dialogs, and actions inherit the Akita surface style.',
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', xl: '0.78fr 1.22fr' }, gap: 2, mb: 2 }}>
        <Surface sx={{ p: 2.5, minHeight: 178 }}>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ maxWidth: 620, mt: 1 }}>
            ...
            button tone, and the Autumn field skin before replacing the installed GUI.
          </Typography>
        ...
        {rows.map((item, index) => (
          <Box key={item} sx={{ py: 1.5, borderTop: `1px solid ${alpha(theme.palette.text.primary, 0.1)}` }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <LinearProgress variant="determinate" value={45 + index * 18} sx={{ width: 140, height: 6, borderRadius: 4 }} />
            </Box>
          </Box>
        ))}
      </Surface>
...
function SandboxScreen() {
  ...
  const metrics = [
    ...
    { label: 'Autumn Plots', value: '42 active', detail: 'ETA 18 days', icon: BarChart },
        ...
        >
          <Toolbar sx={{ minHeight: 84, gap: 2, px: 3 }}>
            <Box flexGrow={1} minWidth={0}>
              <Typography variant="overline" color="text.secondary">
                Akita console - Autumn field / Overview first
              </Typography>
              <Typography variant="h5" fontWeight={700}>
                {selectedPage === 'Overview' ? 'Akita console' : selectedPage}
              </Typography>
            </Box>
            ...
            <Surface sx={{ p: 2.5 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 2, mb: 2 }}>
                <Box>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    Static sample content for Akita console color, spacing, and density checks.
                  </Typography>
                </Box>
                <Button variant="contained" size="small">
                  Mock offer
